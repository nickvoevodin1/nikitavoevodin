...
- Все колонки кроме даты объявлены текстовыми. Без этого Sheets разбирает
  значение так, будто его набрали руками: ведущий «+» он считает синонимом «=»,
  и телефон «+7 914-339-5343» превращается в формулу с `#ERROR!`. Тем же болеют
  ответы, начинающиеся с «-», «=» или «@».
